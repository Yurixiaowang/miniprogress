...
/**
 * 倒计时的到期时间
 *  天 时 分 秒 01天12时30分50秒
 */
@Component
export default class countdown extends Vue {
  @Prop({ type: Number }) expiredTime: number; // expiredTime 过期时间。13位时间戳 ms
  ...
  onAttach() {
    console.log("countdown page onLoad");
  }
  countDown() {
    const leftTime = Math.max(this.expiredTime - Date.now(), 0);
    // TODO 将 leftTime 转换为 day hours mins secs
